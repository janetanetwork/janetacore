...
for Ubuntu Xenial does not produce working executables for some of the Janetacoin Core applications.
...
The next three steps are an example of how to acquire the source in an appropriate way.
    ...
    cd /usr/src
    sudo git clone https://github.com/janetanetwork/janetacoin.git
    sudo chmod -R a+rw janetacoin
...
causes two of the janetacoin executables to crash shortly after start up. The bug is related to the
...
It's not possible to build the janetacoin code using the win32 version of the Mingw-w64 cross compilers (at least not without
modifying headers in the janetacoin source code).
