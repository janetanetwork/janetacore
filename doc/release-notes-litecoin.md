...
or `litecoind`/`janetacoin-qt` (on Linux).
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 dammi per kB (0.00001 JTA). This is to prepare for dropping the minimum transaction fee to 0.1 dammi per kB (0.0001 JTA) in the next release.
